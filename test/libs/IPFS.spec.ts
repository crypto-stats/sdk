import { expect } from 'chai';
import { IPFS } from '../../src/libs/IPFS';

const DATA_URI = 'data:image/svg+xml;base64,';

describe('IPFS', function() {
  let ipfs = new IPFS();

  beforeEach(() => {
    ipfs = new IPFS();
  });

  it('should load a text file from IPFS', async () => {
    const result = await ipfs.getFile('QmSTbSvecWDxM1jPDKBJ2ECBTwwuSAFEyuA111NvMYosr3');

    expect(result).to.equal('test123');
  });

  it('should load a data URI', async () => {
    const uri = await ipfs.getDataURI('QmagaZMuMQKU2a5aJ2VofXBNwHmYpzo8GUR9CCjLWbgxTE', 'image/svg+xml');

    expect(uri).to.equal(DATA_URI);
  });

  it('should generate an IPFS loader', async () => {
    const loader = ipfs.getDataURILoader('QmagaZMuMQKU2a5aJ2VofXBNwHmYpzo8GUR9CCjLWbgxTE', 'image/svg+xml');

    expect(loader.cid).to.equal('QmagaZMuMQKU2a5aJ2VofXBNwHmYpzo8GUR9CCjLWbgxTE');
    expect(loader.mimeType).to.equal('image/svg+xml');

    const uri = await loader();
    expect(uri).to.equal(DATA_URI);
  });
});
